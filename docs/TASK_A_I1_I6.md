...
- `topk_mask(map, k=16) -> sorted coordinate list`; `ring_matched_controls(mask, n=10, seeds=200..209)`: random masks with identical per‑ring counts, drawn from positions outside the primary mask; a test asserts ring composition equality and determinism.
