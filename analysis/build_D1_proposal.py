...
TABLES = "results/frozen/I2_terminal/calibration/tables"
THRESHOLDS = "results/frozen/I2_terminal/calibration/thresholds_cal.json"
LOCKED = "docs/LOCKED_ANALYSIS.md"

#: `Signed <name>, <date>, <git sha>.` as §1 of LOCKED_ANALYSIS records it.
SIGNATURE_RE = re.compile(
    r"^> Signed (?P<name>.+?), (?P<date>\d{4}-\d{2}-\d{2}), "
    r"(?P<sha>[0-9a-f]{7,40})\.", re.MULTILINE)


def read_signature(locked=LOCKED):
    """The D1 signature, READ BACK from `docs/LOCKED_ANALYSIS.md` §1.

    The note is generated, so its status block cannot be a hand edit — but
    the signature is a fact about the LOCKED document, not about this file.
    Reading it back means `build()` reproduces a signed note byte for byte
    and the two can never disagree about who signed what: if §1 says nothing,
    the note says PROPOSED.
    """
    path = Path(locked)
    if not path.exists():                                # pragma: no cover
        return None
    text = path.read_text(encoding="utf-8")
    section = text.split("## 1. Feature stage", 1)
    if len(section) < 2:                                 # pragma: no cover
        return None
    m = SIGNATURE_RE.search(section[1].split("## 2.", 1)[0])
...
def build(tables=TABLES, thresholds=THRESHOLDS, cell=DECISION_CELL,
          locked=LOCKED) -> str:
    tables = Path(tables)
    ...
    signature = read_signature(locked)
    ...
    if signature:
        w("> ## STATUS: **SIGNED**")
        w(">")
        w(f"> Signed and dated by: {signature['name']}  ")
        w(f"> Date signed: {signature['date']}  ")
        w(f"> Git sha at signature: `{signature['sha']}`")
        w(">")
        w(f"> **D1 is CLOSED: `{v['d1_stage']}`.** The wording signed into  ")
        w("> `docs/LOCKED_ANALYSIS.md` §1 is the ALTERNATIVE offered in §3(b) "
          "below,  ")
        w("> not the rule's branch-3 boilerplate; the VERDICT is the rule's, "
          "unchanged.  ")
        w("> I2 Phase B2 and the Phase-B runs of I1/I3/I4/I5 are released.")
        w(">")
        w("> The three lines above are READ BACK from "
          "`docs/LOCKED_ANALYSIS.md` §1, which  ")
        w("> is where the signature lives; this note is generated and holds "
          "no signature  ")
        w("> of its own.")
    else:
        ...
        w("> Until those three lines are filled in and the sentence below is "
          "copied into  ")
        w("> `docs/LOCKED_ANALYSIS.md` §1, **D1 is still `DECISION "
          "NEEDED`**: no I2 Phase-B2  ")
        w("> run and no I1/I3/I4/I5 Phase-B run may start (task file §7, "
          "§10).")
    w("")
