#!/usr/bin/env python3
"""
figures/fig1_voc_candidates.py
==============================

Generates VOC candidate attention map grids for Figure 1 Slot 6.
...
def stage_voc(voc_tar: str, stage_dir: str) -> str:
    """
    Extract VOC tar temporarily.
    Returns VOC root.
    ...
    print("  Staging PASCAL VOC...")

    with tarfile.open(voc_tar, "r:*") as tf:
    ...
    for root in possible_roots:
        if root.exists():
            ...
            if image_dir.exists():
                n = len(list(image_dir.glob("*.jpg"))) + len(list(image_dir.glob("*.jpeg")))
                if n > 0:
                    print(f"  VOC staged: {n} images")
                    return str(root)

    n = len(list(out.rglob("*.jpg"))) + len(list(out.rglob("*.jpeg")))

    if n > 0:
        print(f"  VOC staged: {n} images")
        return str(out)

    raise RuntimeError("Could not find VOC images after extraction.")
...
def get_voc_candidates(voc_root: str, n: int) -> list:
    """
    Select random VOC images.
    """
    root = Path(voc_root)
    ...
    if len(paths) == 0:
        raise RuntimeError(f"No VOC images found under {voc_root}")
...
def main():
    default_stage = str(
        Path(tempfile.gettempdir()) / f"fig1_voc_stage_{os.environ.get('USER', 'user')}"
    )
    ...
    voc_stage = str(Path(args.stage_base) / "voc")

    try:
        print("=" * 55)
        print("  Slot 6 — PASCAL VOC")
        print("=" * 55)
    ...
    print(f"\n{'=' * 55}")
    print("  VOC candidate PNGs saved to:")
    print(f"  {args.out_dir}")
    print()
    print("  Output pattern:")
    print("  candidates_slot6_voc_set01.png")
    print("  candidates_slot6_voc_set02.png")
    print("  candidates_slot6_voc_set03.png")
    print("  candidates_slot6_voc_set04.png")
    print("  candidates_slot6_voc_set05.png")
    print(f"{'=' * 55}\n")
