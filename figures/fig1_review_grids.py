#!/usr/bin/env python3
"""
figures/fig1_review_grids.py
==============================
Creates 10 review figures, each with 3 rows (candidates) × 4 columns.

Reads the top-30 ranked images from scores.csv (output of fig1_select_coco.py).
Groups them into 10 figures of 3 rows each:
    Figure 01: ranks  1-3
    Figure 02: ranks  4-6
    ...
    Figure 10: ranks 28-30

Visual design: identical to fig1_teaser_final.py — same colormap, same
spacing, same borders, same headers.

Usage:
    python3 figures/fig1_review_grids.py \
        --paths_e2   /home/vault/iwi5/iwi5359h/SAGA/e2/checkpoints \
        --coco_zip   /home/woody/iwi5/iwi5359h/Data/COCO/val2017.zip \
        --scores_csv /home/woody/iwi5/iwi5359h/saga_figures/selection/scores.csv \
        --out_dir    /home/woody/iwi5/iwi5359h/saga_figures/selection/grids \
        --top_k      30
"""
...
def stage_selected(coco_zip, filenames, stage_dir):
    """Extract only the needed filenames from COCO val2017.zip."""
    stage_dir = Path(stage_dir)
    stage_dir.mkdir(parents=True, exist_ok=True)
    wanted = set(filenames)
    found  = {}

    print(f"  Extracting {len(wanted)} images from COCO zip...")
    ...
    if missing:
        raise FileNotFoundError(f"Not found in zip: {missing}")

    print(f"  Staged {len(found)} images")
    return found   # {filename: path}
...
# ── 3-row grid figure — identical layout to fig1_teaser_final.py ──────────────

def save_grid_figure(rows_data, fig_idx, out_path):
    """
    rows_data: list of 3 dicts, each with keys:
        display, attn_b, attn_r, attn_s, rank, filename
    fig_idx: 1-based figure index (1..10)
    """
    n_rows = len(rows_data)   # always 3

    # ── Exact same layout as fig1_teaser_final.py ─────────────────────────────
    ...
    for r_idx, row in enumerate(rows_data):
        ...
        # Row label on the left (rank + filename)
...
def main():
    ...
    parser.add_argument("--scores_csv", required=True,
                        help="scores.csv from fig1_select_coco.py")
    parser.add_argument("--out_dir",    required=True)
    parser.add_argument("--top_k",      type=int, default=30,
                        help="How many top images to use (must be divisible by 3)")
    parser.add_argument("--stage_base", default="/tmp/fig1_grids_stage")
    ...
    print(f"Read {len(filenames)} filenames from {args.scores_csv}")

    # ── Stage only the needed images ───────────────────────────────────────────
    stage_dir = Path(args.stage_base)
    if stage_dir.exists():
        shutil.rmtree(stage_dir)
    path_map = stage_selected(args.coco_zip, filenames, stage_dir)
    ...
    # ── Run inference for all 30 images ───────────────────────────────────────
    ...
    for fig_idx in range(1, n_figs + 1):
        ...
        out_path     = out_dir / f"grid_{fig_idx:02d}_ranks{rank_start:02d}-{rank_end:02d}.png"
    ...
    print(f"\nDone. All grids saved to: {out_dir}")
    print("\nFilename reference:")
    for i, row in enumerate(rows):
        print(f"  Rank {row['rank']:>2}  {row['filename']}")
